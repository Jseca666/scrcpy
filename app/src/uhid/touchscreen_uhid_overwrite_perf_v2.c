...
static bool
sc_touchscreen_uhid_slot_matches(const struct sc_touchscreen_uhid *touchscreen,
                                 int slot, uint64_t pointer_id,
                                 bool include_releasing) {
    if (slot < 0 || (unsigned) slot >= SC_HID_TOUCHSCREEN_CONTACTS) {
        return false;
    }

    const struct sc_touchscreen_slot *state = &touchscreen->slots[slot];
    if (!state->active || state->pointer_id != pointer_id) {
        return false;
    }

    return include_releasing
        || (!state->pending_release && !state->finalize_pending_commit);
}

static void
sc_touchscreen_uhid_remember_slot_hint(struct sc_touchscreen_uhid *touchscreen,
                                       int slot, uint64_t pointer_id) {
    touchscreen->last_pointer_slot = (int8_t) slot;
    touchscreen->last_pointer_id = pointer_id;
}

static void
sc_touchscreen_uhid_invalidate_slot_hint(struct sc_touchscreen_uhid *touchscreen,
                                         int slot) {
    if (touchscreen->last_pointer_slot == slot) {
        touchscreen->last_pointer_slot = -1;
        touchscreen->last_pointer_id = 0;
    }
}

static void
sc_touchscreen_uhid_recompute_sole_active_slot(
        struct sc_touchscreen_uhid *touchscreen) {
    if (touchscreen->active_pointer_count != 1) {
        touchscreen->sole_active_slot = -1;
        return;
    }
    ...
    for (unsigned i = 0; i < SC_HID_TOUCHSCREEN_CONTACTS; ++i) {
        if (touchscreen->slots[i].active
            ...
            touchscreen->sole_active_slot = (int8_t) i;
            return;
        }
    }
...
static void
sc_touchscreen_uhid_track_pointer_became_active(
        struct sc_touchscreen_uhid *touchscreen, int slot) {
    ++touchscreen->active_pointer_count;
    touchscreen->sole_active_slot = touchscreen->active_pointer_count == 1
                                  ? (int8_t) slot
                                  : -1;
}

static void
sc_touchscreen_uhid_track_pointer_left_active(
        struct sc_touchscreen_uhid *touchscreen, int slot) {
    if (!touchscreen->active_pointer_count) {
        touchscreen->sole_active_slot = -1;
        return;
    }
    ...
    if (!touchscreen->active_pointer_count) {
        touchscreen->sole_active_slot = -1;
    } else if (touchscreen->active_pointer_count == 1) {
        sc_touchscreen_uhid_recompute_sole_active_slot(touchscreen);
    } else {
        touchscreen->sole_active_slot = -1;
    }

    sc_touchscreen_uhid_invalidate_slot_hint(touchscreen, slot);
}

static int
sc_touchscreen_uhid_find_slot(struct sc_touchscreen_uhid *touchscreen,
                              uint64_t pointer_id, bool include_releasing) {
    if (touchscreen->last_pointer_id == pointer_id
            && sc_touchscreen_uhid_slot_matches(
                    touchscreen, touchscreen->last_pointer_slot, pointer_id,
                    include_releasing)) {
        return touchscreen->last_pointer_slot;
    }

    for (unsigned i = 0; i < SC_HID_TOUCHSCREEN_CONTACTS; ++i) {
        if (!sc_touchscreen_uhid_slot_matches(touchscreen, (int) i, pointer_id,
                                              include_releasing)) {
            continue;
        }
        sc_touchscreen_uhid_remember_slot_hint(touchscreen, (int) i, pointer_id);
        return (int) i;
    }
...
sc_touchscreen_uhid_acquire_slot(struct sc_touchscreen_uhid *touchscreen,
    ...
    for (unsigned i = 0; i < SC_HID_TOUCHSCREEN_CONTACTS; ++i) {
        struct sc_touchscreen_slot *state = &touchscreen->slots[i];
        if (state->active) {
            continue;
        }
        ...
        state->finalize_pending_commit = false;
        state->pointer_id = pointer_id;
        state->contact_id = touchscreen->next_contact_id++;
        if (!state->contact_id) {
            state->contact_id = touchscreen->next_contact_id++;
        }

        sc_touchscreen_uhid_track_pointer_became_active(touchscreen, (int) i);
        sc_touchscreen_uhid_remember_slot_hint(touchscreen, (int) i, pointer_id);
        return (int) i;
    }
...
static void
sc_touchscreen_uhid_clear_slot_pending_release(
        struct sc_touchscreen_uhid *touchscreen, unsigned slot) {
    if (touchscreen->slots[slot].pending_release) {
        touchscreen->slots[slot].pending_release = false;
        if (touchscreen->pending_release_count) {
            --touchscreen->pending_release_count;
        }
    }
}

static void
sc_touchscreen_uhid_mark_slot_pending_release(
        struct sc_touchscreen_uhid *touchscreen, unsigned slot) {
    if (!touchscreen->slots[slot].pending_release) {
...
static void
sc_touchscreen_uhid_clear_slot_finalize_pending(
        struct sc_touchscreen_uhid *touchscreen, unsigned slot) {
    if (touchscreen->slots[slot].finalize_pending_commit) {
        touchscreen->slots[slot].finalize_pending_commit = false;
        if (touchscreen->pending_finalize_count) {
            --touchscreen->pending_finalize_count;
        }
    }
}

static void
sc_touchscreen_uhid_mark_slot_finalize_pending(
        struct sc_touchscreen_uhid *touchscreen, unsigned slot) {
    if (!touchscreen->slots[slot].finalize_pending_commit) {
        sc_touchscreen_uhid_mark_slot_finalize_pending(touchscreen, (unsigned) slot);
        ++touchscreen->pending_finalize_count;
    }
}
...
sc_touchscreen_uhid_release_slot(struct sc_touchscreen_uhid *touchscreen,
    ...
    sc_touchscreen_uhid_clear_slot_pending_release(touchscreen, slot);
    sc_touchscreen_uhid_clear_slot_finalize_pending(touchscreen, slot);
    sc_touchscreen_uhid_invalidate_slot_hint(touchscreen, (int) slot);
...
sc_touchscreen_uhid_store_contact(struct sc_touchscreen_uhid *touchscreen,
    ...
    if (contact->present
            && contact->contact_id == touchscreen->slots[slot].contact_id
            && contact->x == x
            && contact->y == y
            && contact->width == width
...
sc_touchscreen_uhid_finalize_pending_releases(
    ...
    if (!touchscreen->pending_release_count || !touchscreen->pending_finalize_count) {
        return false;
    }
    ...
    for (unsigned i = 0; i < SC_HID_TOUCHSCREEN_CONTACTS; ++i) {
        ...
        sc_touchscreen_uhid_clear_slot_pending_release(touchscreen, i);
...
sc_touchscreen_uhid_reset_runtime_state(struct sc_touchscreen_uhid *touchscreen) {
    ...
    touchscreen->dirty = false;
    touchscreen->active_pointer_count = 0;
    touchscreen->sole_active_slot = -1;
    touchscreen->last_pointer_slot = -1;
    touchscreen->last_pointer_id = 0;
}
...
sc_touchscreen_uhid_pointer_down(struct sc_touchscreen_uhid *touchscreen,
    ...
    if (sc_touchscreen_uhid_store_contact(
                touchscreen, slot, x, y, pressure_value, touch_major,
...
sc_touchscreen_uhid_pointer_move(struct sc_touchscreen_uhid *touchscreen,
    ...
    if (touchscreen->active_pointer_count == 1
            && touchscreen->sole_active_slot == slot) {
...
sc_touchscreen_uhid_pointer_release(struct sc_touchscreen_uhid *touchscreen,
    ...
    sc_touchscreen_uhid_track_pointer_left_active(touchscreen, slot);
    sc_touchscreen_uhid_mark_slot_pending_release(touchscreen, (unsigned) slot);
    sc_touchscreen_uhid_clear_slot_finalize_pending(touchscreen, (unsigned) slot);
...
sc_touchscreen_uhid_finalize_pointer(struct sc_touchscreen_uhid *touchscreen,
    ...
    if (!touchscreen->slots[slot].pending_release
            && !touchscreen->slots[slot].finalize_pending_commit) {
        sc_touchscreen_uhid_track_pointer_left_active(touchscreen, slot);
    }
    sc_touchscreen_uhid_mark_slot_finalize_pending(touchscreen,
                                                   (unsigned) slot);
    ...
    if (!touchscreen->explicit_frame_depth) {
        sc_hid_touchscreen_clear_contact(&touchscreen->hid, (unsigned) slot);
        sc_touchscreen_uhid_clear_slot_pending_release(touchscreen,
                                                       (unsigned) slot);
        sc_touch_simulation_clear_slot(&touchscreen->sampling_slots[slot]);
        sc_touchscreen_uhid_commit_or_defer(touchscreen);
    }
...
sc_touchscreen_uhid_mark_all_releasing(struct sc_touchscreen_uhid *touchscreen,
    ...
    for (unsigned i = 0; i < SC_HID_TOUCHSCREEN_CONTACTS; ++i) {
        ...
        sc_touchscreen_uhid_mark_slot_pending_release(touchscreen, i);
        sc_touchscreen_uhid_mark_slot_finalize_pending(touchscreen, i);
...
sc_touchscreen_uhid_reset(struct sc_touchscreen_uhid *touchscreen) {
    ...
    touchscreen->pending_release_count = 0;
    touchscreen->pending_finalize_count = 0;
